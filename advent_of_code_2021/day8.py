signal_patterns = """cdafg dage fgdaec cdbfgae cge gcbdfa fdceb gfceab ge ecfgd | eg eg dfecag ge
...
signal_patterns = signal_patterns.splitlines()

def one_four_seven_eight(signal_patterns:list[str]) -> int:
    digits = {num: 0 for num in range(10)}
    output_values = [pattern.split(' | ')[1] for pattern in signal_patterns]
...
def decoder(input_code:list) -> dict:
    input_code = [set(code) for code in input_code]
    digits = {}
    ...
    for code in input_code:
        if code in digits.values():
            continue
        
        if code.issuperset(digits[4] | digits[7]) and code != digits[8]:
            digits[9] = code
    ...
    for code in input_code:
        ...
        if code.issubset(digits[9]) and digits[1].issubset(code):
            digits[3] = code
        elif code.issubset(digits[9]) and not digits[1].issubset(code):
            digits[5] = code
    ...
    for code in input_code:
        ...
        if code.issuperset(digits[2] - digits[7]) and code != digits[8]:
            digits[6] = code
    ...
    return {frozenset(value): key for key, value in digits.items()}

def output_sum(signal_patterns:list[str]):
    total = 0
    signal_patterns = [pattern.split(' | ') for pattern in signal_patterns] # [[input1, output1], etc.]
    for input, output in signal_patterns:
        input = input.split()
        output = output.split()
        decodings = decoder(input)
        cur_sum = 0
        for idx, code in enumerate(output[::-1]):
            code = frozenset(code)
            digit = decodings[code] * (10 ** idx)
            cur_sum += digit
        total += cur_sum
    
    return total

print(output_sum(signal_patterns))